...
//	freopen("in.txt","r",stdin);
	...
	cin >> l;
	if(a == b){
		printf("%.2lf\n%.2lf\n",max(dist(a,c)-l,(double)0),max(dist(a,c)-l,(double)0));
		return 0;
	}
	point res;
	double mi = pointSegmentDistance(a, b, c, res);
	double mx = max(dist(a, c), dist(b, c));
	printf("%.2lf\n%.2lf\n", max(mi - l,(double)0), max(mx - l,(double)0));
	return 0;
}
