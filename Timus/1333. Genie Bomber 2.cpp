...
bool inCircle(point p , point c , double r){
	double dx = p.real() - c.real();
	double dy = p.imag() - c.imag();
...
//	freopen("in.txt", "r", stdin);
	int n;
	cin>>n;
	vector<pair<point,double> > cir(n);
	for(int i=0;i<n;i++)
		cin>>cir[i].first.real()>>cir[i].first.imag()>>cir[i].second;
	int cnt=0;
	for(double i=0;i<=1;i+=0.001){
		for(double j=0;j<=1;j+=0.001){
			for(int k=0;k<n;k++){
				if(inCircle(point(i,j),cir[k].first,cir[k].second)){
					cnt++;
					break;
				}
			}
