...
bool insidePoly(point p, vector<point> poly) {
	for (int i = 0; i < sz(poly); i++) {
		if (p == poly[i] || PointOnSegment(poly[i],poly[(i+1)%sz(poly)] , p))
			return true;
	}
...
vector<point>cutPoly(point a , point b , vector<point> poly){
	vector<point>p;
	for(int i=0;i<sz(poly);i++){
		double left1 = cross_p(a,b,poly[i]) , left2=0.0;
		if(i!=sz(poly)-1)
			left2 = cross_p(a,b,poly[i+1]);
		if(left1 > -eps)
			p.push_back(poly[i]);
		if(left1 * left2 < -eps){
			point ret;
			intersectSeg(poly[i],poly[i+1],a,b,ret);
			p.push_back(ret);
		}
	}
		if(p.empty())
			return p;
		if(fabs(p.back().real() - p.front().real()) > eps || fabs(p.back().imag() - p.front().imag() > eps))
			p.push_back(p.front());
	return p;
}
...
int main() {
	...
	while(t--){
		...
		point c = p[0];
		while(tt){
			while(idx < n && dist(c,p[idx]) < have){
				have-=dist(c,p[idx]);
				c = p[idx];
				idx++;
			}
			point current = vec(c,p[idx]);
			current = normalize(current) * have + c;
