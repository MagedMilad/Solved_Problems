...
int main() {
	...
	while(t--){
		...
		for(int i=0;i<n;i++){
			...
			theta *=(-M_PI/180.0);
			sum1+=w*h;
			point p1 = rotateAbout(point(x,y) + point(w/2,h/2),theta,point(x,y));
			point p2 = rotateAbout(point(x,y) + point(-w/2,h/2),theta,point(x,y));
			point p3 = rotateAbout(point(x,y) + point(w/2,-h/2),theta,point(x,y));
			point p4 = rotateAbout(point(x,y) + point(-w/2,-h/2),theta,point(x,y));
			p.push_back(p1);
			p.push_back(p2);
			p.push_back(p3);
			p.push_back(p4);
